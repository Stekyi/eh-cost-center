...
import { DEFAULT_CUSTOMER_CATEGORIES, nextCode } from '../utils/customerSegments'

const PREFIX = 'CAT'

export default function CustomerCategories() {
  const { docs: items, refresh } = useLiveCollection('customerCategories')
  ...
  useEffect(() => {
    async function ensureDefaults() {
      try {
        ...
        const rows = await listDocs('customerCategories')
        ...
        for (const defaultLabel of DEFAULT_CUSTOMER_CATEGORIES) {
          ...
          await createDoc('customerCategories', {
            code,
            label: defaultLabel,
            active: true,
          })
        ...
        console.error('CustomerCategories:ensureDefaults failed', err)
      }
    }
    ensureDefaults()
  ...
  async function createItem(e?: React.FormEvent) {
    ...
    try {
      ...
      console.error('CustomerCategories:create failed', err)
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    try {
      await updateDocById('customerCategories', editing.id, {
      ...
      console.error('CustomerCategories:saveEdit failed', err)
      alert(err?.message || 'Failed to update category')
    }
  }

  async function removeItem(id: string) {
    if (!window.confirm('Delete customer category?')) return
    try {
      await deleteDocById('customerCategories', id)
      refresh()
    } catch (err: any) {
      console.error('CustomerCategories:delete failed', err)
  ...
  const columns = [
    { field: 'label', headerName: 'Category Name', flex: 1, minWidth: 220 },
