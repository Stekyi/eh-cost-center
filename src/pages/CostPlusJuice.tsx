...
export default function CostPlusJuice(){
  ...
  useEffect(()=>{
    async function load(){
      ...
      const filtered = arr.filter(p=> p.type === 'juice')
      setProducts(filtered)
      if(filtered.length && !productId) setProductId(filtered[0].id)
    }
    load()
  },[])
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '4px 8px 32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <h2 style={{ margin: 0 }}>Cost-Plus Calculator — Juices</h2>
